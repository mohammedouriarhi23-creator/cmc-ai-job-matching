...
export default function Rechercher() {
  ...
  const offresStageEtPfe = useMemo(() => offres.filter((o) => o.type !== "Emploi"), [])

  const filtered = useMemo(() => {
    return offresStageEtPfe.filter((o) => {
      const matchQ =
        !filters.q ||
  ...
  }, [filters, offresStageEtPfe])

  return (
    <div>
      <h1 className="mb-1 text-2xl font-extrabold text-gray-900">Rechercher un stage / PFE</h1>
      <p className="mb-6 text-sm text-gray-500">
        {filtered.length} opportunité{filtered.length > 1 ? "s" : ""} de stage ou de PFE
